<?php
if (isset($_POST['nombre']) && (trim($_POST['nombre'])!='') && isset($_POST['email']) && (trim($_POST['email'])!='') && isset($_POST['asunto']) && (trim($_POST['asunto'])!='') && isset($_POST['mensaje'])&& (trim($_POST['mensaje'])!='')) {
	 require_once('./librerias/functions.php'); 

	$cuerpo .= "Los datos de la contacto son:\n";
	$cuerpo .= "Nombre: " . utf8_decode($_POST['nombre']) . "\n";
	if (isset($_POST['tfno']))
		$cuerpo .= "Tfno: " . utf8_decode($_POST['tfno']) . "\n";
	...
	if ($resultado) {
		...
		?> <script language=javascript> window.alert("Lo lamentamos pero el envio ha fallado. Por favor, intentelo de nuevo.\n"); </script> <?php
	}
	?>
